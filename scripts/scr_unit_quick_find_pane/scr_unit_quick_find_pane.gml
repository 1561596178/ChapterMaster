function UnitQuickFindPanel() constructor {
    ...
    static draw_fleet_area = function() {
        ...
        if (hovered_fleet_data != undefined) {
            ...
            // Headers
            draw_text(_sx + 160, _sy + 10, L("forge point\ntotal"));
            draw_text(_sx + 240, _sy + 10, L("forge point\nuse"));
            draw_text(_sx + 320, _sy + 10, L("apothecary\npoint total"));
            draw_text(_sx + 400, _sy + 10, L("apothecary\npoint use"));
            draw_text(_sx + 60, _sy + 50, L("Orbiting"));
        ...
        with (obj_star) {
            for (var i = 1; i <= planets; i++) {
                ...
                for (var p = 0; p < array_length(problems); p++) {
                    ...
                    var mission_explain = L(mission_name_key(problems[p]));
    ...
    main_panel.inside_method = function() {
        ...
        if (view_area == "fleets") {
            ...
            draw_set_halign(fa_center);
            draw_text(xx + 80, yy + 50, L("System"));
            draw_text(xx + 160, yy + 50, L("Troops"));
            draw_text(xx + 240, yy + 50, L("Healers"));
            draw_text(xx + 310, yy + 50, L("Techies"));
            var i = start_system;
            ...
            while (i < array_length(system_names) && (yy + 122 + (20 * i)) < main_panel.YY + yy + main_panel.height) {
                ...
                if (scr_hit(xx + 10, yy + 90 + (20 * i), xx + main_panel.width, _sys_item_y)) {
                    ...
                    if (struct_exists(_special_points, _sys_name)) {
                        ...
                        draw_set_font(fnt_40k_12i);
                        draw_text(_xx + 160, _yy + 10, L("forge point\ntotal"));
                        draw_text(_xx + 240, _yy + 10, L("forge point\nuse"));
                        draw_text(_xx + 320, _yy + 10, L("apothecary\npoint total"));
                        draw_text(_xx + 400, _yy + 10, L("apothecary\npoint use"));
                        draw_text(_xx + 60, _yy + 50, L("Orbiting"));
                        for (var s = 1; s <= 4; s++) {
            ...
            if (obj_controller.menu == eMENU.DEFAULT && !obj_controller.zoomed) {
                if (!instances_exist_any([obj_fleet_select, obj_star_select])) {
                    ...
                    if (hide_sequence > 15 || hide_sequence < 15) {
                        ...
                        if (tab_buttons.fleets.draw(x_draw, 79, L("Fleets"))) {
                            view_area = "fleets";
                            update_fleet_table();
                        }
                        if (tab_buttons.garrisons.draw(115 + x_draw, 79, L("System Troops"))) {
                            view_area = "garrisons";
                            update_garrison_log();
                        }
                        if (tab_buttons.missions.draw(230 + x_draw, 79, L("Missions"))) {
                            view_area = "missions";
                            update_mission_log();
                        }
                        if (x_draw < 0) {
                            tab_buttons.hider.draw(0, lower_draw, L("Show"));
                        } else {
                            if (tab_buttons.hider.draw(x_draw + 280, lower_draw, L("Hide"))) {
                                hide_sequence++;
                            }
                        }
                    } else if (hide_sequence == 15) {
                        if (tab_buttons.hider.draw(0, lower_draw, L("Show"))) {
                            hide_sequence++;
                        }
                    }
...
function HelpfulPlaces() constructor {
    ...
    with (obj_star) {
        ...
        if (_forge > 0) {
            var _data = {
                ...
                owner_name: L(obj_controller.faction[p_owner[_forge]]),
                owner: p_owner[_forge],
                owner_status: L(obj_controller.faction_status[p_owner[_forge]]),
            };
            ...
            _data.hover = method(_data, function() {
                tooltip_draw(LF("click to view {0} system", [system.name]));
            });
    ...
    forges_table = new Table({
        ...
        headings: [L("Name"), L("   Owner   "), L("  Owner\nStatus  ")],
        row_data: _forges,
        set_column_widths: [100, 70, 80],
    });
