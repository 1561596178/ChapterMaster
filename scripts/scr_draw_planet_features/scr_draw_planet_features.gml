    ...
    draw_planet_features = function(xx, yy) {
        ...
        switch (feature.f_type) {
            case eP_FEATURES.FORGE:
                draw_text_transformed(xx + (area_width / 2), yy + 10, L("Chapter Forge"), 2, 2, 0);
                draw_set_halign(fa_left);
                draw_set_color(c_gray);

                draw_text(xx + 10, yy + 50, LF("Working Techs : {0}/{1}", [feature.techs_working, worker_capacity]));
                if (point_and_click(draw_unit_buttons([xx + 10, yy + 70], L("Assign To Forge"), [1, 1], c_red))) {
                    obj_controller.unit_profile = false;
                    obj_controller.view_squad = false;
                    group_selection(techs, {purpose: L("Forge Assignment"), purpose_code: "forge_assignment", number: worker_capacity, system: planet_data.system, feature: feature, planet: planet_data.planet, selections: []});
                    destroy = true;
                }
                ...
                if (feature.size < 3) {
                    ...
                    var last_button = draw_unit_buttons(next_position, LF("Upgrade Forge ({0} req)", [upgrade_cost]), [1, 1], c_red);
                    next_position = [
                        last_button[0],
                        last_button[3],
                    ];
                ...
                if (feature.size > 1 && !feature.vehicle_hanger) {
                    ...
                    var build_coords = draw_unit_buttons(next_position, LF("Build Vehicle Hanger({0} req)", [upgrade_cost]), [1, 1], c_red);
                    if (scr_hit(build_coords)) {
                        tooltip_draw(L("Required to Build Vehicles in the Forge"));
                    }
                    ...
                    draw_text(next_position[0], next_position[1], L("Forge has a vehicle hanger"));
            ...
            case eP_FEATURES.VICTORY_SHRINE:
                draw_text_transformed(xx + (area_width / 2), yy + 10, L("Victory Shrine"), 2, 2, 0);
                draw_set_halign(fa_left);
                draw_set_color(c_gray);
                break;
            case eP_FEATURES.MONASTERY:
                ...
                if (feature.forge == 0) {
                    draw_text_transformed(xx + 80, yy + 50, L("Forge"), 1, 1, 0);
            ...
            case eP_FEATURES.RECRUITING_WORLD:
                ...
                body = LF("There are {0} apothecary rescource points available for recruit screening,\n\n", [_spare_apoth_points]);
                var _recruit_find_chance = find_recruit_success_chance(_spare_apoth_points, _star, _planet, 1);

                body += LF("There is a {0}% of producing a successful recruit this month on the basis of the available apothecary time to screen candidates and the chances of the aspirants passing their trials to an acceptable standard,\n\n", [_recruit_find_chance * 100]);
                ...
                if ((obj_controller.faction_status[p_data.current_owner] == "War" || obj_controller.faction_status[p_data.current_owner] == "Antagonism") && (p_data.player_disposition <= 50)) {
                    ...
                    if (_recruit_world.recruit_type == 0) {
                        body += L("Since our relations with the populations' faction are... strained, we are having to do our recruiting operation covertly,\n\n");
                    } else {
                        body += L("Since our relations with the populations' faction are... strained, we are having to do our recruiting operation covertly,");
                        body += L(" our brothers are authorized to use more extreme methods of recruitment,\n\n");
                    }
                    ...
                    if (_recruit_world.recruit_type == 0) {
                        body += L("The population has grown accustomed to us and their Governor has given us the clear to openly recruit,\n\n");
                    } else {
                        body += L("The population has grown accustomed to us and their Governor has given us the clear to openly recruit,");
                        body += L(" however our brothers are still authorized to use more extreme methods of recruitment regardless,\n\n");
                    }
                } else if (_recruit_world.recruit_type == 1) {
                    body += L("We've authorized our brothers to use more extreme methods of recruitment, should we really allow this Milord?\n\n");
                }

                if (p_data.player_disposition < 100) {
                    body += L("To increase recruit success chance more apothecaries will be required on the planet surface, we could also deploy garrisons to make the population more friendly to our chapter.");
                } else {
                    body += L("To increase recruit success chance more apothecaries will be required on the planet surface.");
                }
                break;
            case eP_FEATURES.MISSION:
                ...
                switch (feature.problem) {
                    case "provide_garrison":
                        ...
                        mission_description = LF("The governor of {0} has requested a force of marines might stay behind following your departure.\n\n\n assign a squad to garrison to initiate mission, The garrison leeader will need to be capable of conducting himself in a diplomatic manner in order for the garrison duration to be a success", [planet_name]);

                        break;
                    case "join_communion":
                        mission_description = LF("The governor of {0} has Invited a delegate of your forces to take part in ceremony.", [planet_name]);
                        break;
                    case "hunt_beast":
                        mission_description = LF("The governor of {0} has bemoaned the raiding of huge beasts on the fringes of the planets largest city, the numbers have swelled recently and are causing huge damage to the planets small economy. You could send a force to intervene, it would provide a fine test of metal for any that partake.", [planet_name]);
                    ...
                    case "protect_raiders":
                        mission_description = LF("The governor of {0} has sent many requests to the sector commander for help with defending against xenos raids on the populace of the planet, the reports seem to suggest the xenos in question are in fact dark eldar.", [planet_name]);
                        help = L("Set a squad to ambush");
                        button_text = L("Send Squad");
                        _button_tooltip = L("milage may vary on playability of this mission progress at your own risk");
                        button_function = function() {
                            var dudes = collect_role_group("all", obj_star_select.target.name);
                        ...
                        break;
                    case "train_forces":
                        mission_description = LF("The governor of {0} fears the planet will not hold in the case of major incursion, it has not seen war in some time and he fears the ineptitude of the commanders available, he asks for aid in planning a thorough plan for defense and schedule of works for a period of at least 6 months.", [planet_name]);
                        help = LF("A task best suited to the more knowledgable or wise of your Commanders", []);
                        button_text = L("Assign Officer");
                        button_function = function() {
                        ...
                        break;
                    case "Purge_enemies":
                        mission_description = LF("The governor of {0} has expressed his distaste of the neighboring governance of {1} {2} he has expressed his views that they engage in heretical ways and harbor xenos enemies though in truth it is more likely that he simply wishes his political enemies disposed of, whatever the case his planet has great economic means and he has made bare his plans to compensate the emperors angels for their aid", [planet_name, target.name, feature.target]);
                        break;
                }
                draw_text_transformed(xx + (area_width / 2), yy + 5, L(mission_name_key(feature.problem)), 2, 2, 0);
                draw_set_halign(fa_left);
                draw_set_color(c_gray);
                draw_text_ext(xx + 10, yy + 72, mission_description, -1, area_width - 20);
                var text_body_height = string_height_ext(string_hash_to_newline(mission_description), -1, area_width - 20);
                if (help != "none") {
                    draw_text_ext(xx + 10, yy + 72 + text_body_height + 10, help, -1, area_width - 20);
                ...
                if (button_text != "none") {
                    ...
                    if (point_and_click(_button)) {
                        if (is_callable(button_function)) {
                            ...
                            tooltip_draw(L("no implemented function"));
                        }
                    }
                }
                break;
        ...
        if (generic) {
            draw_text_ext_transformed(xx + (area_width / 2), yy + 5, L(title), -1, area_width - 20, 2, 2, 0);

            draw_set_halign(fa_left);
            draw_set_color(c_gray);
            draw_text_ext(xx + 10, yy + 72, L(body), -1, area_width - 20);
        }
        return "done";
    };
}
