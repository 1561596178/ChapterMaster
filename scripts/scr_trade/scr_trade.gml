        ...
        if (_success) {
            ...
            with (obj_controller) {
                ...
                if (_rela == "hostile") {
                    ...
                    if (rando == 3) {
                        diplo_text += L("An unreasonable trade, whatever our working relationship might be. I refuse.");
                    }
                }
                if (annoyed[_dip] >= 10) {
                    force_goodbye = 1;
    ...
    static draw_trade_screen = function() {
        ...
        for (var i = 0; i < array_length(demand_options); i++) {
            ...
            if (_opt.number > 0) {
                ...
                if (_opt.max_take > 1) {
                    draw_text(530, _y_offset, LF("{0} : {1}", [L(_opt.label), _opt.number]));
                } else {
                    draw_text(530, _y_offset, L(_opt.label));
                }
                _requested_count++;
            }
        }
