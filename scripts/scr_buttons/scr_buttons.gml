...
function draw_unit_buttons(position, text, size_mod = [1.5, 1.5], colour = c_gray, _halign = fa_center, font = fnt_40k_14b, alpha_mult = 1, bg = false, bg_color = c_black) {
    ...
    var x2;
    var y2;
    var _text = string_hash_to_newline(L(text));
...
function ReactiveString(text_param, x1_param = 0, y1_param = 0, data = {}) constructor {
    ...
    static draw = function() {
        ...
        if (max_width > -1) {
            if (!scale_text) {
                draw_text_ext_outline(x1, y1, _text, -1, max_width, c_black, colour);
            } else {
                if (with_outline) {
                    draw_text_transformed_outline(x1, y1, _text, scale, scale, 0);
                } else {
                    draw_text_transformed(x1, y1, _text, scale, scale, 0);
                }
            }
        } else {
            draw_text_outline(x1, y1, _text, c_black, colour);
        }
        if (hit()) {
...
function ValueShifter(value_text, data = {}) constructor {
    ...
    static update = function(data = {}) {
        move_data_to_current_scope(data, true);
        reactive_string.update({x1, y1, text: $"{L(string_tag)}:{current_value}"});
...
function UnitButtonObject(data = {}) constructor {
    ...
    static draw = function(allow_click = true) {
        ...
        if (style == "standard") {
            ...
            draw_text_transformed(_text_position_x, y1 + h / 2, L(label), text_scale, text_scale, 0);
...
function ToggleButton(data = {}) constructor {
    ...
    static update = function(data = {}) {
        ...
        if (style == "default") {
            if (w == 0) {
                w = string_width(L(str1));
                w *= 1 + (text_padding * 2);
            }
            if (h == 0) {
                h = string_height(L(str1));
                h *= 1 + (text_padding * 2);
            }
        } else if (style == "box") {
            var _text_w = string_width(L(str1)) * (1 + (text_padding * 2));
            w = max(32, _text_w) + 12;
            h = 32 + 4 + (string_height(L(str1)) * (1 + (text_padding * 2)));
        }
        x2 = x1 + w;
        y2 = y1 + h;
        draw_set_font(temp_font);
...
draw_text_color_simple(text_x, text_y, L(str1), text_color, total_alpha);
            ...
            draw_text_transformed(_center_x, _label_y, L(str1), 1, 1, 0);
            draw_set_alpha(1);
        }
