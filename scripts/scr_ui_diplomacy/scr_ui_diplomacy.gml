...
function draw_character_diplomacy_base_page() {
    ...
    if (!audience && !valid_diplomacy_options()) {
        with (diplo_buttons) {
            ...
            denounce.draw();
            praise.draw();
            declare_war.draw_shutter(praise.x1, alliance.y2, L("WAR"), 0.4);
        }
    } else {
...
function set_up_diplomacy_persons() {
    ...
    for (var i = 0; i < array_length(_shutters); i++) {
        ...
        with (_button) {
            ...
            cover_text = L(obj_controller.faction[_faction_enum]);
            inside_method = function() {
                var yy = YY;
                var xx = XX;
                draw_set_font(fnt_40k_14b);
                draw_set_halign(fa_left);
                draw_text_transformed(xx + 169, yy + 30, L(obj_controller.faction[_faction_enum]), 0.7, 0.7, 0);
                management_buttons.screen_slate.draw_with_dimensions(xx + 5, yy + 5);
                draw_set_font(fnt_40k_14);
                draw_set_halign(fa_right);
                draw_text_transformed(xx + 420, yy + 34, L(obj_controller.faction_status[_faction_enum]), 0.7, 0.7, 0);
                draw_set_halign(fa_left);
                var txt = "????";
                if (obj_controller.known[_faction_enum] > 0) {
                    txt = $"{L(obj_controller.faction_title[_faction_enum])} {obj_controller.faction_leader[_faction_enum]}";
                }
                draw_text_transformed(xx + 169, yy + 50, txt, 0.7, 0.7, 0);
                draw_text_transformed(xx + 169, yy + 65, LF("Disposition: {0}", [obj_controller.disposition[_faction_enum]]), 0.7, 0.7, 0);
...
function scr_ui_diplomacy() {
    ...
    if (diplomacy == 0) {
        ...
        draw_set_halign(fa_center);
        draw_text(xx + 800, yy + 74, string_hash_to_newline(L("Diplomacy")));
        ...
        draw_set_color(c_black);
        draw_text_transformed(xx + 688, yy + 241, L("Meet Chaos Emissary"), 0.7, 0.7, 0);
    ...
    if (diplomacy > 0) {
        ...
        var _diplomacy_faction_alligience = " (" + L("Imperium") + ")";
        var _disposition_rating = "";
        ...
        if (daemon) {
            draw_text_transformed(xx + 622, yy + 104, L("The Emmmisary"), 0.6, 0.6, 0);
            show_stuff = true;
        } else {
        ...
        draw_set_font(fnt_40k_14);
        if (!daemon) {
            _disposition_rating = LF("Disposition: {0} ({1})", [faction_disposition_rating_string(diplomacy), disposition[diplomacy]]);
    ...
    if (warning == 1 || diplomacy >= 6) {
        var warn = L("Consorting with heretics will cause your disposition with the Imperium to plummet.");
        if (array_contains(global.xenos_factions, diplomacy)) {
            warn = L("Consorting with xenos will cause your disposition with the Imperium to lower.");
        }
