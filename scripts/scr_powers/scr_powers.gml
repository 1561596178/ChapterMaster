...
function player_select_powers() {
    if (race[100][17] != 0) {
        ...
        draw_text_transformed(440, 660, L(_title_01), 0.6, 0.6, 0);
        if (scr_hit(440, 660, 440 + string_width(L(_title_01)) * 0.6, 690)) {
            tooltip = L(_title_01);
            tooltip2 = L("The Psychic Discipline that your psykers will use by default.");
        }

        var _discipline_name = get_discipline_data(discipline, "name");
        draw_text_transformed(520, 697, L(_discipline_name), 0.5, 0.5, 0);

        var _psy_info = get_discipline_data(discipline, "description");
        draw_text_transformed(440, 729, L(_psy_info), 0.5, 0.5, 0);
