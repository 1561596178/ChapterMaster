...
function scr_enemy_ai_d() {
    ...
    for (var i = 1; i <= planets; i++) {
        ...
        if ((p_halp[i] == 0) && (p_population[i] > 0) && (p_owner[i] <= eFACTION.ECCLESIARCHY) && (p_owner[i] != eFACTION.PLAYER) && (present_fleet[1] <= 0) && (p_player[i] <= 0)) {
            ...
            if (enemies == 1) {
                p_halp[i] = 1;
                tx = LF("The Planetary Governor of {0} requests help against {1} forces!", [planet_numeral_name(i, id), L(enemy1)]);
                scr_alert("green", "halp", string(tx), x, y);
                scr_event_log("", string(tx), name);
            }
            if (enemies > 1) {
                p_halp[i] = 1;
                tx = LF("The Planetary Governor of {0} {1} requests help against numerous enemy forces!", [name, scr_roman(i)]);
                scr_alert("green", "halp", string(tx), x, y);
                scr_event_log("", string(tx), name);
            }
        }
