...
function drop_select_unit_selection() {
    ...
    if (purge == eDROP_TYPE.RAIDATTACK) {
        ...
        var attack_type = attack ? L("Attacking") : L("Raiding");
        draw_text_transformed(x1 + 40, y1 + 38, LF("{0} ({1})", [attack_type, planet_numeral_name(planet_number, p_target)]), 0.6, 0.6, 0);
        var _offset = x1 + 40;
        ...
        // Formation
        var _formation_str = LF("Formation: {0}", [L(obj_controller.bat_formation[formation_possible[formation_current]])]);
        ...
        draw_set_color(CM_GREEN_COLOR);
        draw_text(x1 + 40, 273, L("Available Forces:"));
    }
    ...
    draw_text(_squads_box.x1, _squads_box.y1, L(_squads_box.header));
    var _x_offset = 0;
    var _row = 0;
    ...
    if (purge == eDROP_TYPE.RAIDATTACK) {
        ...
        if (race_quantity != 0) {
            _target_str = $"{L(target_race)} ({L(target_threat)})";
        }
            ...
            if (purge == 1) {} else if (purge >= 2) {
                ...
                if (!succession) {
                    if ((p_target.dispo[pp] >= 0) && (p_target.p_first[pp] <= eFACTION.ECCLESIARCHY) && (p_target.p_owner[pp] <= eFACTION.ECCLESIARCHY) && (p_target.p_population[pp] > 0)) {
                        draw_text(x2 + 231, y2 + 54, string_hash_to_newline(L("Disposition: ") + string(min(100, p_target.dispo[pp])) + "/100"));
                    }
                    if ((p_target.dispo[pp] > -30) && (p_target.dispo[pp] < 0) && (p_target.p_owner[pp] <= eFACTION.ECCLESIARCHY) && (p_target.p_population[pp] > 0)) {
                        draw_text(x2 + 231, y2 + 54, string_hash_to_newline(L("Disposition: ???/100")));
                    }
                    if (((p_target.dispo[pp] >= 0) && (p_target.p_first[pp] <= eFACTION.ECCLESIARCHY) && (p_target.p_owner[pp] > eFACTION.ECCLESIARCHY)) || (p_target.p_population[pp] <= 0)) {
                        draw_text(x2 + 231, y2 + 54, string_hash_to_newline(L("-------------")));
                    }
                    if (p_target.dispo[pp] <= -3000) {
                        draw_text(x2 + 231, y2 + 54, L("Chapter Rule"));
                    }
                }
                if (succession == 1) {
                    draw_text(x2 + 231, y2 + 54, L("War of Succession"));
                }
