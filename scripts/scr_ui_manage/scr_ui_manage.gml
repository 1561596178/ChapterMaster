...
function command_slot_draw(xx, yy, slot_text) {
    ...
    draw_text(xx + 500, yy + 66, $"++{L(slot_text)}++");
...
function reset_manage_unit_constants(unit) {
    try {
        ...
        unit_manage_constants.psy = new LabeledIcon(spr_icon_psyker, _psionic, 0, 0, {
            icon_width: 24,
            icon_height: 24,
            tooltip: $"=={L("Psychic Stats")}==\n{_tooltip}",
        });
        // Damage Resistance

        var _res_tool = L("Health damage taken by the marine is reduced by this percentage. This happens after the flat reduction from armor.\n\nContributing factors:\n");
        var equipment_types = [
            "armour",
            "weapon_one",
            "weapon_two",
        ...
        var _hp_tool = L("A measure of how much punishment the creature can take. Marines can go into the negatives and still survive, but they'll require a bionic to become fighting fit once more.\n\nContributing factors:\n");
        ...
        var _armour_tool = L("Reduces incoming damage at a flat rate. Certain enemies may attack in ways that may bypass your armor entirely, for example power weapons and some warp sorceries.\n\nContributing factors:\n");
        ...
        unit_manage_constants.exp = new LabeledIcon(spr_icon_veteran, string(floor(unit.experience)), 0, 0, {
            icon_width: 24,
            icon_height: 24,
            tooltip: LF("==Experience==\n{0}", [L("A measurement of how battle-hardened the unit is. Provides various bonuses across the board. Every 15 EXP, a new stat is assigned. Hover over the unit’s stats in the marine profile to see projected growth over time.")]),
        });
        ...
        var _bionic_tool = L("Bionic Augmentation is something a unit can do to both enhance their capabilities, but also replace a missing limb to get back into the fight.");
        ...
        for (var part = 0; part < array_length(_body_parts); part++) {
            if (struct_exists(unit.body[$ _body_parts[part]], "bionic")) {
                ...
                _bionic_tool += $"{L("Bionic")} {L(part_display)}";
                switch (part_display) {
                    case "Left Leg":
                    case "Right Leg":
        ...
        if (unit.company <= 0) {
            ...
            _comp_string = LF("{0} Company", [unit.company_roman()]);
...
function company_specific_management() {
    ...
    if (managing > 20) {
        ...
        _company_name = LF("{0} Company", [int_to_roman(managing)]);
        _comp = managing;
    } else if (managing > 10) {
        switch (managing) {
            case 11:
                _company_name = L("Headquarters");
                break;
            case 12:
                _company_name = L("Apothecarion");
                break;
            case 13:
                _company_name = L("Librarium");
                break;
            case 14:
                _company_name = L("Reclusium");
                break;
            case 15:
                _company_name = L("Armamentarium");
                break;
        }
    }
    // Draw the company followed by chapters name
    draw_text(800, 46, $"{_company_name}, {global.chapter_name}");
    if (managing <= 10) {
...
function alternative_manage_views(x1, y1) {
    ...
    _squad_button.update({x1: x1 + 5, y1: y1 + 6, label: !obj_controller.view_squad && !obj_controller.company_report ? L("Squad View") : L("Company View"), keystroke: keyboard_check_pressed(ord("S")) && allow_shortcuts});
    ...
    if ((menu == 1) && (managing > 0 || managing < 0)) {
        ...
        if (managing < 0 && selection_data != false) {
            if (struct_exists(selection_data, "purpose")) {
                draw_text(xx + 800, yy + 74, $"{L(selection_data.purpose)}");
            }
        ...
        if (managing > 0) {
            if (managing >= 1 && managing <= 10) {
                fx = LF("{0} Company", [int_to_roman(managing)]);
            } else if (managing > 10) {
                switch (managing) {
                    case 11:
                        fx = L("Headquarters");
        ...
        for (var sel = top; sel < repetitions && sel < array_length(sh_name); sel++) {
            if (sh_name[sel] != "") {
                ...
                draw_set_color(c_gray);
                draw_rectangle(xx + 25, yy + 64, xx + 974, yy + 85, 1);
                draw_text_transformed(xx + 27, yy + 63, temp1, 1, 1, 0);
                draw_text_transformed(xx + 27.5, yy + 63.5, temp1, 1, 1, 0);
                draw_text_transformed(xx + 364, yy + 63, string(temp2), 1, 1, 0);
                draw_text_transformed(xx + 580, yy + 63, string(temp3), 1, 1, 0);
                draw_text_transformed(xx + 730, yy + 63, string(temp4), 1, 1, 0);
        ...
        // Load to selected
        draw_set_font(fnt_40k_14b);
        draw_text_transformed(xx + 320, yy + 402, LF("Click a Ship to Load Selection (Req. {0} Space)", [man_size]), 1, 1, 0);
...
function draw_manage_selection_buttons(xx, yy) {
    ...
    if (sel_uni[1] != "") {
        ...
        draw_set_font(fnt_40k_30b);
        draw_text_transformed(actions_block.x1 + 26, actions_block.y1 + 6, LF("Selection: {0} space", [man_size]), 0.5, 0.5, 0);
