...
function draw_chapter_select() {
    ...
    draw_text(800, 80, L("Select Chapter"));

    draw_set_font(fnt_40k_30b);
    draw_set_halign(fa_left);

    draw_text_transformed(440, founding_y, L("Founding Chapters"), 0.75, 0.75, 0);
    draw_text_transformed(440, successor_y, L("Existing Chapters"), 0.75, 0.75, 0);
    draw_text_transformed(440, custom_y, L("Custom Chapters"), 0.75, 0.75, 0);
    draw_text_transformed(440, other_y, L("Other"), 0.75, 0.75, 0);
    ...
    if ((tool == 1) && (change_slide <= 0)) {
        ...
        if (highlight <= array_length(all_chapters)) {
            ...
            if (chap.progenitor != 0 && chap.progenitor < 10) {
                tooltip += LF("  - Progenitor: {0}", [L(all_chapters[chap.progenitor].name)]);
            }
            tooltip2 = L(chap.tooltip);
        }
        if (highlight == 1001) {
            tooltip = L("Custom");
        }
        ...
        if (highlight == 1001) {
            tooltip2 = L("Create your own customized Chapter, deciding the origins, strength, and weaknesses.  Custom Chapters are weaker than Founding Chapters.");
        }
        if (highlight == 1002) {
            tooltip2 = L("Randomly generate a Chapter to play.  The origins, strength, and weaknesses are all random.  Random Chapters are normally weaker than Founding Chapters. ");
        }
    }
}
...
function draw_chapter_trait_select() {
    ...
    if ((text_selected != "chapter") || (custom != eCHAPTER_TYPE.CUSTOM)) {
        draw_text_transformed(800, 74, string_hash_to_newline(string(chapter_name)), 0.8, 0.8, 0);
    }
    if (custom == eCHAPTER_TYPE.CUSTOM) {
        ...
        if ((text_selected == "chapter") && (text_bar <= 30)) {
            draw_text_transformed(805, 74, string_hash_to_newline(string(chapter_name) + "|"), 0.8, 0.8, 0);
        }
        if (scr_text_hit(800, 74, true, chapter_name)) {
        ...
        draw_set_alpha(0.75);
        draw_rectangle(580, 74, 1020, 106, 1);
        draw_set_alpha(1);
    }

    draw_set_color(CM_GREEN_COLOR);
    draw_text_transformed(800, 120, string_hash_to_newline(L("Points: ") + string(points) + "/" + string(maxpoints)), 0.6, 0.6, 0);
    ...
    if (popup == "") {
        ...
        draw_set_halign(fa_center);
        draw_text_transformed(800, 301, L("Chapter Stats"), 0.6, 0.6, 0);
        draw_set_halign(fa_left);
        ...
        draw_text_transformed(505, 332, LF("Strength: {0} ({1})", [L(_strength_ratings[strength]), strength]), 0.5, 0.5, 0);
        draw_text_transformed(505, 387, LF("Cooperation: {0}  ({1})", [L(_cooperation_ratings[cooperation]), cooperation]), 0.5, 0.5, 0);
        draw_text_transformed(505, 442, LF("Gene-Seed Purity: {0} ({1})", [L(_geneseed_ratings[purity]), purity]), 0.5, 0.5, 0);
        draw_text_transformed(505, 497, LF("Gene-Seed Stability: ({0}%)", [stability]), 0.5, 0.5, 0);
        ...
        if (custom == eCHAPTER_TYPE.CUSTOM) {
            for (var i = 0; i < 4; i++) {
                ...
                if (scr_hit(436, 325 + (i * 55), 436 + sprite_get_width(spr_arrow), 357 + (i * 55))) {
                    obj_cursor.image_index = 1;
                    tooltip = L("Decrease");
                    tooltip2 = L("(Hold Ctrl to decrease by 10)");
                ...
                if (scr_hit(470, 325 + (i * 55), 470 + sprite_get_width(spr_arrow), 357 + (i * 55))) {
                    obj_cursor.image_index = 1;
                    tooltip = L("Increase");
                    tooltip2 = L("(Hold Ctrl to increase by 10)");
    ...
    if (popup != "icons") {
        ...
        if (scr_hit(436, 564, 631, 583)) {
            tooltip = L("Chapter Advantages");
            tooltip2 = L("Advantages cost points, and improve the performance of your Chapter in a specific domain. You can only have one trait of the same category, shown in brackets.");
        }
        if (scr_hit(810, 564, 1030, 583)) {
            tooltip = L("Chapter Disadvantages");
            tooltip2 = L("Disadvantages grant additional points, and penalize the performance of your Chapter. You can only have one trait of the same category, shown in brackets.");
        }
    } else if (popup == "icons") {
        draw_set_alpha(1);
        draw_set_color(c_black);
        ...
        draw_set_font(fnt_40k_30b);
        draw_set_halign(fa_center);
        draw_text_transformed(800, 211, L("Select an Icon"), 0.6, 0.6, 0);
        draw_text_transformed(800, 687, L("Cancel"), 0.6, 0.6, 0);

        var cw = string_width(L("Cancel")) * 0.6;
        var ch = string_height(L("Cancel")) * 0.6;
...
function draw_chapter_homeworld_select() {
    ...
    left_data_slate.inside_method = function() {
        if (!buttons.complex_homeworld.active) {
            ...
            draw_set_halign(fa_center);
            draw_text_transformed(160, 90, L("Aspirant Trial"), 0.6, 0.6, 0);

            if (custom == eCHAPTER_TYPE.CUSTOM) {
                var _title_w = string_width(L("Aspirant Trial")) * 0.6;
                draw_sprite_stretched(spr_creation_arrow, 0, 160 - (_title_w / 2) - 42, 90, 32, 32);
                if (point_and_click([160 - (_title_w / 2) - 42, 90, 160 - (_title_w / 2) - 10, 90 + 32])) {
                    aspirant_trial++;
                    if (aspirant_trial >= array_length(trial_data)) {
                        aspirant_trial = 0;
                    }
                }
                var _right_x = 160 + (_title_w / 2) + 10;
            ...
            draw_text_transformed(160, 132, L(current_trial.name), 0.5, 0.5, 0);

            var asp_info = scr_compile_trial_bonus_string(current_trial);

            draw_set_halign(fa_center);

            draw_text_ext_transformed(160, 172, L(asp_info), -1, left_data_slate.width - 20, 0.4, 0.4, 0);

            if (scr_hit(50, 480, 950, 510)) {
                tooltip = L("Aspirant Trial");
                tooltip2 = L("A special challenge is needed for Aspirants to be judged worthy of becoming Astartes.  After completing the Trial they then become a Neophyte, beginning implantation and training.  (This can be changed once in game, but the chosen trial here will affect the spawn characteristics of your starting Space Marines).");
            }
        } else {
            draw_set_font(fnt_40k_30b);
