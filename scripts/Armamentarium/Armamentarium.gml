...
function STCResearchPanel(_controller_ref, _on_change_callback) constructor {
    ...
    static refresh_eta = function() {
        ...
        if (_points_per_turn <= 0) {
            advisor_eta_text = L("Research: Stalled (No Research Points)");
            return;
        }
        ...
        advisor_eta_text = LF("Research: Next {0} breakthrough in {1} months.", [L(_focus), _months]);
    };
    ...
    static _draw_research_column = function(_cat, _label, _cx, _cy) {
        ...
        if (scr_hit(_rect)) {
            ...
            tooltip_draw(LF("Click to focus research on {0}", [L(_label)]));
        ...
        draw_text(_cx + LAYOUT.LABEL_X_OFFSET, _cy + LAYOUT.HEADER_Y_OFFSET, L(_label));
    ...
    static _draw_bonus_list = function(_cat, _tx, _ty, _current_level) {
        ...
        for (var s = 0; s < array_length(_bonuses); s++) {
            ...
            draw_text_ext_transformed(_tx, _ty + (s * LAYOUT.ROW_SPACING), LF("{0}) {1}", [s, L(_bonuses[s])]), -1, 140, 0.85, 0.85, 0);
        }
        draw_set_alpha(1.0);
    };
    ...
    static _draw_fragment_header = function(_x, _y) {
        ...
        draw_set_font(fnt_aldrich_12);
        draw_set_color(c_gray);
        draw_text(_x + 34, _y, LF("{0} Unidentified Fragments", [_total_un]));
...
function Armamentarium(_controller) constructor {
    ...
    enter_forge_button.cover_text = L("FORGE");
    ...
    static _update_advisor_report = function() {
        ...
        var _text = L("Subject ID confirmed. Rank Identified. Salutations Chapter Master. The status report is ready.");
        _text += LF("\n\nPersonnel: {0}s: {1}, Aspirants: {2}.", [L(_role_tech), count_techmarines, count_aspirants]);
        _text += L("\n\nTraining: ");

        if (controller.faction_status[eFACTION.MECHANICUS] != "War") {
            _text += (_diff > 0) ? LF("We can train {0} more {1}(s).", [_diff, L(_role_tech)]) : LF("To train more, we need {0} more Mechanicus Disposition.", [_req_dispo]);
        } else {
            _text += L("Training handled internally due to Mechanicus hostilities.");
        }

        var _pace = controller.training_techmarine;
        _text += LF(" The training pace is {0}.", [L(global.recruitment_pace_descriptions[_pace] ?? "unknown")]);
        ...
        if (controller.tech_aspirant > 0 && _pace > 0) {
            ...
            _text += LF(" An Aspirant will finish training in {0} month(s).", [_eta_val]);
        }
    ...
    static _draw_header = function() {
        ...
        draw_set_color(c_gray);
        draw_text_transformed(352, 66, is_in_forge ? L("Forge") : L("Armamentarium"), 0.75, 0.75, 0);

        draw_set_font(fnt_aldrich_12);
        var _sub = _is_adept ? $"{L("Adept")} {controller.adept_name}" : $"{L("Forge Master")} {obj_ini.name[0][1]}";
        draw_text(352, 126, _sub);
    };
    ...
    static _draw_slate_contents = function() {
        ...
        var _header_y = 159;
        draw_text(962, _header_y, L("Name"));
        if (shop_type != "technologies") {
            draw_text(1280, _header_y, L("Stocked"));
        }
        draw_text(1410, _header_y, $"{is_in_forge ? L("FP") : L("RP")} Cost");
        ...
        for (var i = _start_index; i < _end_index; i++) {
            ...
            draw_text_color_simple(962, _draw_y_local + 2, L(_display_name), _display_color, _alpha);
    ...
    static _draw_advisor_text = function() {
        draw_set_font(fnt_aldrich_12);
        draw_set_color(c_gray);
        draw_text_ext(352, 155, advisor_report_text, 15, 500);

        var _btn_y = 225 + string_height_ext(advisor_report_text, -1, 500);
        if (enter_forge_button.draw_shutter(526, _btn_y, L("Enter Forge"), 0.5)) {
            is_in_forge = true;
            if (shop_type == "ships") {
                _switch_tab("weapons");
            } else {
    ...
    static _draw_forge_interface = function() {
        var _btn = draw_unit_buttons([659, 82], L("BACK"), [1, 1], CM_GREEN_COLOR,,,,, c_black);
        ...
        var _text = L("Status Report:\n\n");
        _text += LF("Forge Point production per turn: {0}\n", [controller.forge_points]);
        _text += LF("Chapter Total {0}s: {1}\n\n", [L(_role_name), count_total]);
        _text += LF("Planetary Forges in operation: {0}\n\n", [_forge_count]);
        _text += LF("Master Craft Forge Chance: {0}%\n", [_master_craft]);
        _text += L("Assign techmarines to forges to increase Master Craft Chance");
