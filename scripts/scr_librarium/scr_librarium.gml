...
function scr_librarium_gui() {
    ...
    if (artifact_count() == 0) {
        draw_text(622, 440, L("[No Artifacts]"));
        artifact_destroy.draw_shutter(765, 740, L("DESTROY"), 0.3, false);
        artifact_equip.draw_shutter(385, 740, L("EQUIP"), 0.3, false);
        artifact_gift.draw_shutter(575, 740, L("GIFT"), 0.3, false);
        pop_draw_return_values();
        exit;
    }
    /// @type {Struct.ArtifactStruct}
    var cur_arti = fetch_artifact(menu_artifact);
    var artif_descr = LF("This artifact is an unidentified {0}.##It is stored on {1} '{2}'.", [cur_arti.get_type_name(), cur_arti.get_ship_id() > -1 ? L("the ship") : "", cur_arti.get_location_string()]);
...
function scr_librarium() {
    ...
    if (menu_adept == 0) {
        ...
        draw_text_transformed(xx + 352, yy + 66, L("Librarium"), 0.75, 0.75, 0);
        draw_text_transformed(xx + 352, yy + 112, string_hash_to_newline(L("Chief ") + L(obj_ini.role[100][17]) + " " + string(obj_ini.name[0][4])), 0.6, 0.6, 0);
    ...
    if (menu_adept == 1) {
        ...
        draw_text_transformed(xx + 352, yy + 66, L("Librarium"), 0.75, 0.75, 0);
        draw_text_transformed(xx + 352, yy + 112, string_hash_to_newline(L("Adept ") + string(obj_controller.adept_name)), 0.6, 0.6, 0);
        draw_set_font(fnt_40k_14);
    }
    ...
    if (training_psyker >= 0 && training_psyker <= 6) {
        ...
        blurp += L(_recruit_pace[training_psyker]);
    }

    var artif = "";

    if (unused_artifacts == 0) {
        artif = L("no unused artifacts.");
    } else if (unused_artifacts == 1) {
        artif = L("one unused artifact.");
    } else if (unused_artifacts > 1) {
        artif = string(unused_artifacts) + L(" unused artifacts.");
    }

    // Greetings message
    if (menu_adept == 0) {
        draw_text_ext_transformed(xx + 352, yy + 148, string_hash_to_newline(LF("Chapter Master {0}, greetings.#I assume you've come for the report?  The Chapter currently possesses {1} Epistolaries, {2} Codiceries, and {3} Lexicanum.  We are working to identify additional warp-sensitive brothers before they cause harm, and the training is {4}.##We could likely speed up the identification and application of appropriate training, but we would need more resources...I don't suppose we can spare some?##Our Chapter has {5}", [obj_ini.name[0][0], temp[36], temp[37], temp[38], blurp, artif])), -1, 536, 0.85, 0.85, 0);
    }
    if (menu_adept == 1) {
        draw_text_ext_transformed(xx + 352, yy + 148, string_hash_to_newline(LF("Your Chapter contains {0} {1}s, {2} Codiceries, and {3} Lexicanum.##Training of more {1}s is {4}.##Your chapter has {5}", [temp[36], obj_ini.role[100][17], temp[37], temp[38], blurp, artif])), -1, 536, 0.85, 0.85, 0);
    }
    ...
    if (artifact_count() > 0) {
        ...
        draw_text(xx + 622, yy + 440, LF("[Artifact {0} of {1}]", [usey, artifact_count()]));
        ...
        if (scr_hit(xx + 342, yy + 426, xx + 903, yy + 818)) {
            ...
            if (arrow_hovered) {
                tooltip_draw(L("Click here or use mouse wheel to scroll the artifact list."));
            }
            ...
            if (arrow_hovered) {
                tooltip_draw(L("Click on this arrow or use mouse wheel to scroll the artifact list."));
            }
        }
...
function equip_artifact_popup_draw() {
    ...
    draw_set_halign(fa_center);
    draw_text(951 + 312, 48 + 26, LF("Equip Artifact ({0})", [arti.get_display_name()]));
