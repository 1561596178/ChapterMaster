function scr_draw_unit_stat_data(manage = false, data_block = {x1: 1008, y1: 520, w: 569, h: 303}, squeezed = false) {
    ...
    stat_display_list = [
        [
            L("Measure of how quick and nimble unit is as well as their base ability to manipulate and do tasks with their hands.##Influences Ranged Attack"),
            "dexterity",
        ],
        [
            L("How strong a unit. Strong units can wield heavier equipment without penalties and are more deadly in close combat.##Influences Melee Attack#Influences Melee Burden Cap#Influences Ranged Burden Cap"),
            "strength",
        ],
        [
            L("Unit's general toughness and resistance to damage.##Influences Health#Influences Damage Resistance"),
            "constitution",
        ],
        [
            L("Measure of learnt knowledge and specialist skill aptitude.##Influences esoteric knowledge and use of force weapons"),
            "intelligence",
        ],
        [
            L("Unit's perception and street smarts including certain types of battlefield knowledge.##Influences tactical decisions and garrison effects"),
            "wisdom",
        ],
        [
            L("Unit's faith in their given religion or general aptitude towards faith.##Influences resistance to corruption"),
            "piety",
        ],
        [
            L("General skill with close combat weaponry.##Influences Melee Attack#Influences Melee Burden Cap"),
            "weapon_skill",
        ],
        [
            L("General skill with ballistic and ranged weaponry.##Influences Ranged Attack#Influences Ranged Burden Cap"),
            "ballistic_skill",
        ],
        [
            L("...Luck..."),
        ...
        [
            L("Skill and understanding of technology and various technical thingies and ability to interact with the machine spirit.##Influences Forge point output"),
            "technology",
        ],
        [
            L("General likeability and ability to interact with people.##Influences disposition increases and decreases#Influences ability to spread corruption"),
            "charisma",
        ],
    ];
    for (var i = 0; i < array_length(stat_display_list); i++) {
        ...
        draw_text((_attribute_box.x1 + _attribute_box.x2) / 2, _attribute_box.y1 + 28, $"{self[$ _stat_key]}");
        ...
        var _final_string = _stat_description + L("#Click to order by highest ") + L(_stat_name) + LF("#{0}% chance of growth", [stat_percentage]);
    ...
    var _data_lines = [];
    var _data_entry = {};
    _data_entry.text = LF("Loyalty: {0}", [loyalty]) + "\n";
    _data_entry.tooltip = L("Loyalty represents the unwavering devotion to one's Chapter, their Primarch, and the Emperor himself. It is a measure of their ability to resist the temptations of Chaos, the influence of xenos artifacts, and the machinations of the Warp.");
    array_push(_data_lines, _data_entry);

    _data_entry = {};
    _data_entry.text = LF("Corruption: {0}", [corruption]) + "\n";
    _data_entry.tooltip = L("Corruption reflects exposure to the malevolent forces of the Warp. High Corruption may indicate that the person is teetering on the brink of damnation, while a low score suggests relative purity.");
    array_push(_data_lines, _data_entry);

    _data_entry = {};
    _data_entry.text = LF("Assignment: {0} ({1})", [_psionic_assignment, psionic]) + "\n";
    _data_entry.tooltip = L("The Imperium measures and records the psionic activity and power level of psychic individuals through a rating system called The Assignment. Comprised of a twenty-four point scale, The Assignment simplifies the comparison of psykers to aid Imperial authorities in recognizing possible threats.");
    array_push(_data_lines, _data_entry);
    ...
    if (array_length(traits) != 0) {
        for (var i = 0; i < array_length(traits); i++) {
            ...
            var _trait_description = L(_trait.flavour_text);
            var _trait_effect = "";
            if (struct_exists(_trait, "effect")) {
                _trait_effect = L(_trait.effect + ".") + "\n\n";
            }
            var y1 = _attribute_box.y2 + 16 + (i * 24);
            draw_set_halign(fa_right);
            draw_text(x1, y1, L(_trait_name));
            draw_set_halign(fa_left);
            var x2 = x1 - string_width(L(_trait_name));
            var y2 = y1 + string_height(L(_trait_name));
            ...
            for (var j = 0; j < array_length(_stat_list); j++) {
                ...
                if (struct_exists(_trait, _stat)) {
                    ...
                    _trait_growth_effect += LF("{0} : {1}", [L(_stat_name), _descriptive_string]) + "\n";
                }
            }
        ...
        draw_set_halign(fa_right);
        draw_text(data_block.x2 - 16, _attribute_box.y2 + 16, L("No Traits"));
        draw_set_halign(fa_left);
    }
