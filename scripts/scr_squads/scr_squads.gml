...
function localize_squad_display(_disp) {
    ...
    for (var i = 0; i < array_length(_prefixes); i++) {
        ...
        if (string_pos(_p + " ", _disp) == 1) {
            var _rest = string_copy(_disp, string_length(_p) + 2, string_length(_disp));
            var _t = L(_p);
            if (_t != _p) {
                return _t + " " + _rest;
            }
...
function SquadArrangementEditor(company) constructor {
        ...
        for (var i = 0; i < array_length(_squad_keys); i++) {
            ...
            if (!_already_present) {
                ...
                var _btn = new UnitButtonObject({
                    style: "pixel",
                    label: localize_squad_display(_squad_data.type_data.display_data),
                    tooltip: LF("add {0} as a {1} squad", [_key, localize_squad_display(require_filter ? "required" : "proportional")]),
                    set_width: true,
                    x1: picker_x,
                    y1: _py,
                    w: column_w,
    ...
    static add_new_required_type = function(required_squad) {
        ...
        var _squad_display = localize_squad_display(_squad_data.type_data.display_data);
        ...
        var delete_button = new UnitButtonObject({
            style: "pixel",
            label: L("remove squad"),
            tooltip: LF("remove {0} from required squads", [_squad_display]),
            set_width: true,
            x1: required_editor_box.x1,
            y1: box.y2,
            w: required_editor_box.w,
    ...
    static add_new_proportional_type = function(required_squad) {
        ...
        var delete_button = new UnitButtonObject({
            style: "pixel",
            label: L("remove squad"),
            tooltip: LF("remove {0} from proportional squads", [_squad_display]),
            set_width: true,
            x1: _px,
            y1: box.y2,
            w: column_w,
