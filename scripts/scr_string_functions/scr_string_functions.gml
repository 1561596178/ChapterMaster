...
function scr_convert_company_to_string(company_num, possessive = false, flavour = false) {
    ...
    var _str_company = possessive ? L("Company's") : L("Company");

    if ((_company_num < 1) || (_company_num > 10)) {
        return "";
    } else {
        var _flavour_text = flavour ? L(_flavours[_company_num - 1]) : "";
        var _converted_string = string_join(" ", string(_company_num), _flavour_text, _str_company);
