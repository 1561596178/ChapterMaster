function PlayerPurge(action_type, action_score, planet_data) constructor {
    ...
    static population_death_string = function() {
        ...
        switch (action_type) {
            case eDROP_TYPE.PURGEBOMBARD:
                _death_string += L("were purged over the duration of the bombardment.");
                break;
            case eDROP_TYPE.PURGEFIRE:
                _death_string += L(" over the duration of the cleansing.");
                break;
            case eDROP_TYPE.PURGESELECTIVE:
                _death_string += L(" over the duration of the search.");
                break;
        }

        _death_string += L(" were purged");

        switch (heres_target) {
            case "corruption":
                _death_string += LF("\n\nHeresy has fallen to {0}%.", [heres_after]);
                break;
            case "tau":
                _death_string += LF("\nTau influence is now effecting {0}% of the population.", [heres_after]);
                break;
            case "genestealers":
                _death_string += LF("\nGenestealer influence is now effecting {0}% of the population.", [heres_after]);
                break;
        }
...
function scr_purge_world(action_type, action_score) {
    ...
    if (action_type == eDROP_TYPE.PURGEFIRE) {
        ...
        if (_isquest) {
            ...
            _popup_text = choose(LF("Timing their visits right, Your forces scour {0} burning down whatever the local heretic communities call their homes. Their screams were quickly extinguished by fire, turning whatever it was before, into ash.", [name()]), LF("Your forces scour {0}, burning homes and towns that reek of heresy. The screams and wails of the damned carry through the air.", [name()]));
