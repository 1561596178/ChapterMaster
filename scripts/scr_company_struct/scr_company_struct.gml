...
function CompanyStruct(comp) constructor {
    ...
    static draw_squad_assignment_options = function() {
        ...
        if (_cur_squad.assignment == "none") {
            draw_text_transformed(xx + bound_width[0] + 5, yy + bound_height[0] + 125, L("Squad has no current assignments"), 1, 1, 0);
            ...
            draw_text_transformed(xx + bound_width[0] + 5, yy + bound_height[0] + 125, LF("Assignment : {0}", [cur_assignment.type]), 1, 1, 0);
            var tooltip_text = L("Cancel Assignment");
    ...
    static draw_squad_view = function() {
        ...
        if (squad_leader != "none") {
            ...
            var leader_text = is_struct(_fetched) ? LF("Squad Leader : {0}", [_fetched.name_role()]) : L("Squad Leader : Unknown");
            draw_text_transformed(xx + bound_width[0] + 5, yy + bound_height[0] + 50, leader_text, 1, 1, 0);
        }
        squad_loc = _cur_squad.squad_loci();
        draw_text_transformed(xx + bound_width[0] + 5, yy + bound_height[0] + 75, LF("Squad Members : {0}", [_cur_squad.life_members]), 1, 1, 0);
        draw_text_transformed(xx + bound_width[0] + 5, yy + bound_height[0] + 100, LF("Squad Location : {0}", [squad_loc.text]), 1, 1, 0);
        ...
        var deploy_text = L("Squad will deploy in the");
        if (_cur_squad.formation_place != "") {
            ...
            draw_text_transformed(xx + bound_width[0] + 5 + string_width(deploy_text) + string_width(_cur_squad.formation_place) + 9, yy + bound_height[0], L("column"), 1, 1, 0);
            draw_set_color(c_gray);
            var _dd_extend = 0;
            if (array_length(_cur_squad.formation_options) > 1) {
                ...
                if (drop_down_open) {
                    ...
                    for (var col = 0; col < array_length(_cur_squad.formation_options); col++) {
                        ...
                        button = draw_unit_buttons([button[0], button[3] + 2], L(_cur_squad.formation_options[col]), [1, 1], c_red,,,, true);
                        _dd_extend += button[3] - button[1] + 2;
            ...
            bound_height[0] += drop_down_open ? _dd_extend : (button[3] - button[1]);
        }

        var _dd_offset = drop_down_open ? _dd_extend : 0;
        reset_loadout_button.y1 += _dd_offset;
        reset_loadout_button.y2 += _dd_offset;
        ...
        mass_equip_toggle.y1 += _dd_offset;
        mass_equip_toggle.y2 += _dd_offset;
