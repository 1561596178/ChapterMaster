...
function scr_fleet_advisor() {
    ...
    if (menu_adept == 0) {
        ...
        draw_text_transformed(xx + 352, yy + 66, L("Flagship Bridge"), 0.75, 0.75, 0);
        draw_text_transformed(xx + 352, yy + 112, $"{L("Master of the Fleet")} {ini.lord_admiral_name}", 0.6, 0.6, 0);
        draw_set_font(fnt_40k_14);
        _blurp = L("Greetings, Chapter Master.\n\nYou requested a report?  Our fleet contains ");
    }
    if (menu_adept == 1) {
        ...
        draw_set_color(c_gray);
        draw_set_font(fnt_40k_30b);
        draw_text_transformed(xx + 352, yy + 40, L("Flagship Bridge"), 0.75, 0.75, 0);
        draw_text_transformed(xx + 352, yy + 112, $"{L("Adept")} {cn.adept_name}", 0.6, 0.6, 0);
        draw_set_font(fnt_40k_14);
        _blurp = L("Your fleet contains ");
    }

    _blurp += string(temp[37]) + L(" Capital Ships, ");
    _blurp += string(temp[38]) + L(" Frigates, and ");
    _blurp += string(temp[39]) + L(" Escorts");

    var _hull_normalized = real(temp[41]);

    if (_hull_normalized >= 1) {
        _blurp += L(", none of which are damaged.");
    } else if (_hull_normalized < 1) {
        _blurp += LF(".  Our most damaged vessel is the {0} - it has {1}% Hull Integrity.", [temp[40], min(99, round(_hull_normalized * 100))]);
    }

    var _crippled_ships = real(temp[42]);

    if (_crippled_ships == 2) {
        _blurp += L("  Two of our ships are highly damaged.  You may wish to purchase a Repair License from the Sector Governerner.");
    } else if (_crippled_ships > 2) {
        _blurp += L("  Several of our ships are highly damaged.  It is advisable that you purchase a Repair License from the Sector Governer.");
    }
    ...
    draw_text_ext_transformed(xx + 352, yy + 150, _blurp, -1, 536, 0.85, 0.85, 0);

    draw_set_font(fnt_40k_30b);
    draw_set_halign(fa_center);
    draw_text_transformed(xx + 1262, yy + 40, L("Fleet"), 0.6, 0.6, 0);
    ...
    if (instance_exists(cn)) {
        ...
        for (var i = 0; i < array_length(_columns_array); i++) {
            with (_columns[$ _columns_array[i]]) {
                ...
                draw_text(header_x, header_y, L(text));
            }
        }
        draw_set_halign(fa_left);
        ...
        for (var i = ship_current; i < ship_current + 34; i++) {
            ...
            if (ini.ship[i] != "") {
                ...
                if (scr_hit(xx + 950, _row_y, xx + 1546, _row_y + _row_height)) {
                    ...
                    tooltip_draw(LF("Carrying ({0}): {1}", [cn.temp[118], cn.temp[119]]));
        ...
        if (cn.temp[101] != "") {
            ...
            draw_text(xx + 383, yy + 655, LF("Health: {0}/{1}", [cn.temp[103], cn.temp[104]]));
            draw_text(xx + 588, yy + 655, LF("Shields: {0}", [cn.temp[105]]));
            draw_text(xx + 768, yy + 655, LF("Armour: {0},{1}", [cn.temp[107], cn.temp[108]]));

            draw_text(xx + 495, yy + 675, LF("Speed: {0}", [cn.temp[106]]));
            draw_text(xx + 680, yy + 675, LF("Turrets: {0}", [cn.temp[109]]));

            if (cn.temp[110] != "") {
                draw_text(xx + 383, yy + 705, LF("-{0} ({1})", [L(cn.temp[110]), L(cn.temp[111])]));
            }
            if (cn.temp[112] != "") {
                draw_text(xx + 383, yy + 725, LF("-{0} ({1})", [L(cn.temp[112]), L(cn.temp[113])]));
            }
            if (cn.temp[114] != "") {
                draw_text(xx + 383, yy + 745, LF("-{0} ({1})", [L(cn.temp[114]), L(cn.temp[115])]));
            }
            if (cn.temp[116] != "") {
                draw_text(xx + 383, yy + 765, LF("-{0} ({1})", [L(cn.temp[116]), L(cn.temp[117])]));
            }
