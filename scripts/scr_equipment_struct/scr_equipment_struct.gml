    ...
    static item_tooltip_desc_gen = function() {
        ...
        if (type == "") {
            ...
            if (struct_exists(global.gear[$ "armour"], name)) {
                ...
                item_desc_tooltip = L("Error: Item not found!");
        ...
        for (var i = 0; i < array_length(stat_order); i++) {
            ...
            switch (stat) {
                case "description":
                    if (description != "") {
                        item_desc_tooltip += $"{L(description)}##";
                    }
                    break;
                case "quality":
                    if (quality != "") {
                        item_desc_tooltip += $"{L("Quality")}: {quality_string_conversion(quality)}##";
                    }
                    break;
                case "armour_value":
                    if (armour_value != 0) {
                        if (item_type == "armour") {
                            item_desc_tooltip += $"{L("Armour")}: {armour_value}#";
                        } else {
                            item_desc_tooltip += $"{L("Armour")}: {format_number_with_sign(armour_value)}#";
                        }
                    }
                    break;
                case "hp_mod":
                    if (hp_mod != 0) {
                        item_desc_tooltip += $"{L("Health Mod")}: {format_number_with_sign(hp_mod)}%#";
                    }
                    break;
                case "damage_resistance_mod":
                    if (damage_resistance_mod != 0) {
                        item_desc_tooltip += $"{L("Damage Res")}: {format_number_with_sign(damage_resistance_mod)}%#";
                    }
                    break;
                case "attack":
                    if (attack != 0) {
                        item_desc_tooltip += $"{L("Damage")}: {attack}#";
                    }
                    break;
                case "spli":
                    if (item_type == "weapon") {
                        item_desc_tooltip += $"{L("Max Kills")}: {max(1, spli)}#";
                    }
                    break;
                case "ranged_mod":
                    if (ranged_mod != 0) {
                        item_desc_tooltip += $"{L("Ranged Mod")}: {format_number_with_sign(ranged_mod)}%#";
                    }
                    break;
                case "melee_mod":
                    if (melee_mod != 0) {
                        item_desc_tooltip += $"{L("Melee Mod")}: {format_number_with_sign(melee_mod)}%#";
                    }
                    break;
                case "ammo":
                    if (ammo != 0) {
                        item_desc_tooltip += $"{L("Ammo")}: {ammo}#";
                    }
                    break;
                case "range":
                    if (range > 1.1) {
                        item_desc_tooltip += $"{L("Range")}: {range}#";
                    }
                    break;
                case "melee_hands":
                    if (melee_hands != 0) {
                        if (item_type == "weapon") {
                            item_desc_tooltip += $"{L("Melee Burden")}: {melee_hands}#";
                        } else {
                            item_desc_tooltip += $"{L("Melee Burden Cap")}: {format_number_with_sign(melee_hands)}#";
                        }
                    }
                    break;
                case "ranged_hands":
                    if (ranged_hands != 0) {
                        if (item_type == "weapon") {
                            item_desc_tooltip += $"{L("Ranged Burden")}: {ranged_hands}#";
                        } else {
                            item_desc_tooltip += $"{L("Ranged Burden Cap")}: {format_number_with_sign(ranged_hands)}#";
                        }
                    }
                    break;
                case "special_properties":
                    ...
                    if (arp == 4) {
                        array_push(special_properties_array, L("Anti Vehicle"));
                    } else if (arp == 1) {
                        array_push(special_properties_array, L("Low Penetration"));
                    } else if (arp == 2) {
                        array_push(special_properties_array, L("Medium Penetration"));
                    } else if (arp == 3) {
                        array_push(special_properties_array, L("High Penetration"));
                    }
                    ...
                    if (_array_length > 0) {
                        ...
                        item_desc_tooltip += $"#{L("Properties")}:#{special_properties_string}#";
                    }
                    break;
                case "special_description":
                    if (special_description != "") {
                        item_desc_tooltip += $"#{L(special_description)}#";
                    }
                    break;
                case "req_exp":
                    if (req_exp > 0) {
                        item_desc_tooltip += $"#{L("Requires")} {req_exp} {L("EXP")}#";
                    }
                    break;
                case "tags":
                    if (array_length(tags) > 0) {
                        ...
                        item_desc_tooltip += $"#{L("Keywords")}:#{tagString}#";
                    }
                    break;
                case "maintenance":
                    if (maintenance > 0) {
                        item_desc_tooltip += $"{L("Maintenance")}: {maintenance}#";
                    }
                    break;
            }
        }
...
function quality_string_conversion(quality_request) {
    ...
    if (struct_exists(quality_conversions, quality_request)) {
        return L(quality_conversions[$ quality_request]);
    } else {
        return "";
    }
}
