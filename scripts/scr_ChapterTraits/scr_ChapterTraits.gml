...
function ChapterTrait(trait) constructor {
    ...
    static effects_string = function() {
        ...
        if (is_array(faction_disp_mods) && array_length(faction_disp_mods) > 0) {
            for (var i = 0; i < array_length(faction_disp_mods); i++) {
                ...
                if (struct_exists(_mod, "int_mod") && _mod.int_mod != 0) {
                    _line += L("  Disposition Gains : ") + string_plus_minus(_mod.int_mod) + string(_mod.int_mod) + "\n";
                }
                ...
                if (struct_exists(_mod, "mult") && _mod.mult != 1) {
                    ...
                    _line += L("  Disposition Multiplyers x") + string(_mod.mult) + "\n";
                }
                ...
                if (struct_exists(_mod, "start_disp") && _mod.start_disp != 0) {
                    ...
                    _line += L("  Disposition Start ") + string_plus_minus(_mod.start_disp) + string(_mod.start_disp) + "\n";
                }

                if (_line != "") {
                    _str += L("Faction ") + _fac_names[_mod.faction] + ":\n" + _line + "\n";
                }
            }
        }
        ...
        if (suspicion != 0) {
            _str += L("Suspicion: ") + string_plus_minus(suspicion) + string(suspicion) + "\n";
        }

        if (array_length(character_spawn_increase)) {
            _str += L("Increases Character trait spawns : ") + string(character_spawn_increase) + "\n";
        }

        if (array_length(character_spawn_decrease)) {
            _str += L("Decrease Character trait spawns : ") + string(character_spawn_decrease) + "\n";
        }
        return _str;
    };

    static main_tool_tip = function() {
        return LF("{0} ({1})", [L(name), points]);
    };

    static data_tool_tip = function() {
        return L(description) + " \n" + L("Categories: ") + print_meta() + "\n\n" + L("Effects:") + "\n" + effects_string();
    };
    ...
    static print_meta = function() {
        if (array_length(meta) == 0) {
            ...
            var _translated = [];
            for (var i = 0; i < array_length(meta); i++) {
                array_push(_translated, L(meta[i]));
            }
            return string_join_ext(", ", _translated);
        }
    };
}
...
function draw_chapter_trait_list(type) {
    ...
    draw_text_transformed(800, 211, LF("Select a {0}", [L(_title)]), 0.6, 0.6, 0);
    ...
    for (var slot = 0; slot < array_length(_list); slot++) {
        ...
        draw_text(column.x1, column.y1 + gap, L(_trait_name));

        var dis_width = string_width(L(_trait_name));
...
function draw_selected_chapter_traits(type) {
    ...
    draw_set_halign(fa_left);
    draw_set_font(fnt_40k_30b);
    draw_text_transformed(_title_x, 564, LF("Chapter {0}", [L(_title)]), 0.5, 0.5, 0);
    ...
    for (var i = 0; i < array_length(_advarray); i++) {
        ...
        if (_adv.activated) {
            if (_advantages < _max_advantage_count) {
                _array = draw_unit_buttons([_adv_txt.x1, _adv_txt.y1 + (_advantages * _adv_txt.h)], LF("[-] {0}", [L(_adv.name)]), [0.75, 0.75], CM_GREEN_COLOR);
                _advantages++;
            } else {
                _adv.remove();
                continue;
        ...
        tooltip = LF("{0} ({1} Points)", [L(_adv.name), _adv.points]);
        tooltip2 = L(_adv.description);
    ...
    for (var i = _advantages; i < _max_advantage_count; i++) {
        ...
        if (scr_hit(_array)) {
            if (bool(type) && points >= maxpoints) {
                tooltip = L("Insufficient Points");
                tooltip2 = L("Add disadvantages or decrease Chapter Stats");
            }
        } else {
            continue;
        }
