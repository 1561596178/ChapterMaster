...
function player_recruit_planet_selection() {
    ...
    with (obj_creation) {
        ...
        _recruit_home.max_width = 300;
        _recruit_home.x_gap = 6;
        ...
        draw_text_transformed(1377, 378, L(recruiting), 0.5, 0.5, 0);
...
function scr_creation_home_planet_create() {
    add_draw_return_values();
    var fleet_type_text = fleet_type == ePLAYER_BASE.HOME_WORLD ? L("Homeworld") : L("Flagship");
    ...
    if (fleet_type == ePLAYER_BASE.HOME_WORLD) {
        scr_image("ui/planet", _cur_planet_index, 596, 250, 96, 96);

        draw_text_transformed(644, 360, L(homeworld), 0.5, 0.5, 0);
        if ((text_selected != "home_name") || (custom != eCHAPTER_TYPE.CUSTOM)) {
            draw_text_transformed(644, 384, homeworld_name, 0.5, 0.5, 0);
        }

        if (custom == eCHAPTER_TYPE.CUSTOM) {
            if (text_selected == "home_name") {
                draw_text_transformed(644, 384, homeworld_name + (text_bar > 30 ? "" : "|"), 0.5, 0.5, 0);
            }

            if (scr_text_hit(644, 384, true, homeworld_name)) {
                obj_cursor.image_index = 2;
                if (mouse_button_clicked()) {
            ...
            draw_set_alpha(0.75);
            draw_rectangle(525, 384, 760, 404, 1);
            draw_set_alpha(1);
        ...
        if (custom == eCHAPTER_TYPE.CUSTOM) {
            draw_sprite_stretched(spr_creation_arrow, 0, 525, 298, 32, 32);
            draw_sprite_stretched(spr_creation_arrow, 1, 725, 298, 32, 32);
            homeworld = list_traveler(planet_types, homeworld, [525, 298, 557, 330], [725, 298, 757, 330]);
        }
    ...
    if (fleet_type != ePLAYER_BASE.HOME_WORLD) {
        ...
        draw_text_transformed(644, 378, L("Battle Barge"), 0.5, 0.5, 0);
    ...
    if (recruiting_exists == 0 && homeworld_exists == 1) {
        ...
        draw_set_alpha(0.5);
        draw_text_transformed(977, 378, L(homeworld), 0.5, 0.5, 0);
    ...
    if (scr_hit(575, 216, 710, 242)) {
        if (fleet_type != ePLAYER_BASE.HOME_WORLD) {
            tooltip = L("Battle Barge");
            tooltip2 = L("The name of your Flagship Battle Barge.");
        } else if (fleet_type == ePLAYER_BASE.HOME_WORLD) {
            tooltip = L("Homeworld");
            tooltip2 = L("The world that your Chapter's Fortress Monastery is located upon.  More civilized worlds are more easily defensible but the citizens may pose a risk or be a nuisance.");
        }
    }
    if (scr_hit(895, 216, 1075, 242)) {
        tooltip = L("Recruiting World");
        tooltip2 = L("The world that your Chapter selects recruits from.  More harsh worlds provide recruits with more grit and warrior mentality.  If you are a homeworld-based Chapter, you may uncheck 'Recruiting World' to recruit from your homeworld instead.");
    }
    ...
    if (fleet_type == ePLAYER_BASE.HOME_WORLD) {
        ...
        draw_text_transformed(445, 480, L("Homeworld Rule"), 0.6, 0.6, 0);
        ...
        for (var i = 0; i < array_length(_homeworld_types); i++) {
            ...
            draw_text_transformed(_draw_x + 40, _draw_y, L(_home_rule_type.name), 0.5, 0.5, 0);
            draw_sprite(spr_creation_check, _cur_select, _draw_x, _draw_y);
            if (scr_hit(_draw_x, _draw_y, _draw_x + 32, _draw_y + 32)) {
                tooltip = L(_home_rule_type.tooltip);
                tooltip2 = L(_home_rule_type.tooltip2);
