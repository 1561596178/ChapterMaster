try {
    ...
    local_content_slate.inside_method = function() {
        ...
        if (purge != eDROP_TYPE.RAIDATTACK) {
            if (purge == eDROP_TYPE.PURGESELECT) {
                ...
                var _exit = draw_unit_buttons([_xx + (_width / 2) - 40, 559], L("Cancel"));
    ...
    roster_slate.inside_method = function() {
        ...
        if (purge == 0) {
            draw_text_ext(_xx, _yy, L("Battle Roster"), -1, roster_slate.width - 40);
            _yy += 30;
            draw_text_ext(_xx, _yy, roster.roster_string, -1, roster_slate.width - 40);
        } else if (purge > eDROP_TYPE.PURGESELECT) {
            draw_text_ext(_xx, _yy, L("Purge Insight"), -1, roster_slate.width - 40);
            _yy += 30;
            var poppy = "0";
            ...
            draw_text(_xx + 14, _yy + 8, LF("Heresy: {0}%", [max(hers, influ[eFACTION.TAU])]));
            draw_text(_xx + 14, _yy + 34, LF("Population: {0}", [poppy]));
        } else if (purge == eDROP_TYPE.PURGESELECT) {
            draw_text_ext(_xx, _yy, L("Purge"), -1, roster_slate.width - 40);
            _yy += 30;
            for (var i = 0; i < array_length(purge_options); i++) {
