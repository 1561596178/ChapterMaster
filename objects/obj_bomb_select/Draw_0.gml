...
if ((max_ships > 0) && instance_exists(obj_star_select)) {
    ...
    draw_text_transformed(bomb_window.x1 + 18, bomb_window.y1 + 30, L("Initializing Bombardment..."), 0.8, 0.8, 0);

    // Target info
    draw_set_font(fnt_info);
    draw_text_bold(bomb_window.x1 + 20, bomb_window.y1 + 70, LF("Target planet: {0}", [p_data.name()]));

    var _total_hers = p_data.corruption + p_data.secret_corruption;
    var str = 0;
    var _influ = p_data.population_influences;

    var population_string = LF("Population: {0} people", [p_data.display_population()]);
    ...
    if (p_target.sprite_index != spr_star_hulk) {
        ...
        var target_string = L("Target force:  ");
        draw_text_bold(bomb_window.x1 + 20, bomb_window.y1 + 110, target_string);
        // 加载提示字符宽度测量用原字符串，故 target_string 保持 key 原文
        ...
        var strength_string = LF("Strength: {0}", [str]);
    ...
    var curr_sel_string = LF("Current Selection: {0} ships", [sel]);
    draw_text_bold(bomb_window.x1 + 20, bomb_window.y2 - 28, curr_sel_string);

    draw_text_bold(bomb_window.x1 + 20, bomb_window.y1 + 160, L("Select ships:"));
    ...
    bombard_button = draw_unit_buttons([bomb_window.x2 - 96, bomb_window.y2 - 40], L("Confirm"), [1, 1], #bf4040, fa_center, fnt_40k_14b, button_alpha);
    ...
    var cancel_button = draw_unit_buttons([bomb_window.x2 - 166, bomb_window.y2 - 40], L("Cancel"), [1, 1], #34bc75, fa_center, fnt_40k_14b);
