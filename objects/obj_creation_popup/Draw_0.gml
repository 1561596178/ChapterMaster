try {
    ...
    if (col_shift) {
        ...
        if (equip) {
            ...
            if (scr_hit(444, 550, 820, 550 + _height)) {
                ...
                tooltip = L("Astartes Role Name");
                tooltip2 = LF("The name of this Astartes Role.  The plural form will be ''{0}s''.", [_role_name]);
            ...
            for (var _slot_count = 0; _slot_count <= 4; _slot_count++) {
                ...
                var _title = $"{L(get_slot_name(type - 100, _slot_count))}: ";
                ...
                draw_text(600, y5, string_hash_to_newline(L(string(_equipment_slot))));
            }
            ...
            if (scr_hit(434, 591, 594, 709)) {
                tooltip = L("Gear");
                tooltip2 = L("The equipment this Astartes Role defaults to.  Note that if defaults are set to expensive items the Astartes may instead be provided with more usual equipment.");
            }
        }
    }
    ...
    if (target_gear > -1) {
        ...
        draw_text_transformed(862, 215, LF("Select {0}", [slot_name]), 0.6, 0.6, 0);
        draw_set_font(fnt_40k_14b);
        ...
        if (array_length(item_name) > max_visible_rows) {
            if (mouse_wheel_down()) {
                popup_scroll = min(array_length(item_name) - max_visible_rows, popup_scroll + 1);
            }
            if (mouse_wheel_up()) {
                popup_scroll = max(0, popup_scroll - 1);
            }
            draw_text_transformed(1008, 215, $"{popup_scroll + 1}/{array_length(item_name)}", 0.5, 0.5, 0);
            draw_set_halign(fa_right);
            draw_text_transformed(1166, 700, L("(Use Mouse Wheel to Scroll)"), 0.5, 0.5, 0);
            draw_set_halign(fa_left);
        }

        for (var h = popup_scroll; h < min(array_length(item_name), popup_scroll + max_visible_rows); h++) {
            var _item_y = y3 + ((h - popup_scroll) * space);
            if (_item_y > max_y) {
                break;
            }
            draw_set_color(CM_GREEN_COLOR);
            var scale = string_width(L(item_name[h])) >= 140 ? 0.65 : 0.8;
            draw_text_transformed(x3, _item_y, L(item_name[h]), scale, scale, 0);

            if (scr_hit(x3, _item_y, x3 + 143, _item_y + 18)) {
                draw_set_color(c_white);
                draw_set_alpha(0.2);
                draw_text_transformed(x3, _item_y, string_hash_to_newline(L(item_name[h])), scale, scale, 0);
                draw_set_alpha(1);
        ...
        if (target_gear == eEQUIPMENT_SLOT.WEAPON_ONE || target_gear == eEQUIPMENT_SLOT.WEAPON_TWO) {
            ...
            for (var h = popup_scroll; h < min(array_length(item_name), popup_scroll + max_visible_rows); h++) {
                ...
                draw_set_color(CM_GREEN_COLOR);
                var scale = string_width(item_name[h]) >= 140 ? 0.65 : 0.8;
                var _button = draw_unit_buttons([x3, _item_y], item_name[h], [scale, scale], CM_GREEN_COLOR);
    ...
    if ((tooltip != "") && (obj_creation.change_slide <= 0)) {
        ...
        if (_box_y + 44 + _tt2_h > room_height - 30) {
            _box_y = mouse_y - 44 - _tt2_h - 20;
        }

        draw_rectangle(_box_x, _box_y, _box_x + _box_w, _box_y + 44 + _tt2_h, 0);
        draw_set_color(CM_GREEN_COLOR);
        draw_rectangle(_box_x, _box_y, _box_x + _box_w, _box_y + 44 + _tt2_h, 1);
        draw_set_font(fnt_40k_14b);
        draw_text(_box_x + 4, _box_y + 2, string_hash_to_newline(string(tooltip)));
        draw_set_font(fnt_40k_14);
        draw_text_ext(_box_x + 4, _box_y + 22, string_hash_to_newline(string(tooltip2)), 18, 500);
    }
} catch (ex) {
