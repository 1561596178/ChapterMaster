if (help == 0) {
    if (instance_exists(obj_controller) && (obj_controller.menu == eMENU.EVENT_LOG)) {
        ...
        draw_text(xx + 800, yy + 74, string_hash_to_newline(LF("{0} Event Log", [global.chapter_name])));
        draw_set_halign(fa_left);
        var ent = array_length(event);
        draw_set_color(CM_GREEN_COLOR);
        if (ent == 0) {
            draw_text(xx + 25, yy + 120, L("No entries logged."));
        } else {
            var p = -1;
            ...
            for (var t = top - 1; t < ent; t++) {
                ...
                if (cur_event.text != "") {
                    ...
                    draw_text_ext(xx + 25, yy + 120 + (p * 26), $"{cur_event.date}  (Turn {cur_event.turn}) - {L(cur_event.text)}", -1, 1554);
                    if (cur_event.event_target != noone) {
                        if (point_and_click(draw_unit_buttons([xx + 1400, yy + 120 + (p * 26)], L("View"), [1, 1], c_green,, fnt_40k_14b, 1, true))) {
...
if (help == 1) {
    ...
    draw_set_halign(fa_left);
    draw_text(xx + 466 + 4, yy + 136 + 6, string_hash_to_newline(L("Topics")));
    var x1 = xx + 466;
    var y1 = yy + 166;
    for (var t = 1; t <= 20; t++) {
        if (topics[t] != "") {
            ...
            draw_text(x1 + 2, y1 + 2, string_hash_to_newline(L(topics[t])));
    ...
    if (topic != "") {
        draw_set_font(fnt_40k_14b);
        draw_text_transformed(xx + 897, yy + 131, string_hash_to_newline(L(topic)), 1.25, 1.25, 0);
        draw_set_halign(fa_left);
        if (info != "") {
            draw_text(xx + 663, yy + 177, string_hash_to_newline(L("Game Info:")));
        }
        draw_set_font(fnt_40k_14);
        var y2 = 0;
        var p1 = L(info);
        ...
        if (strategy != "") {
            ...
            p1 = L(strategy);
            draw_set_font(fnt_40k_14b);
            draw_text(xx + 663, yy + 177 + y2, string_hash_to_newline(L("Strategy:")));
            draw_set_font(fnt_40k_14);
            y2 += 20;
        ...
        if (main_info != "") {
            ...
            p1 = L(main_info);
            draw_set_font(fnt_40k_14b);
            draw_text(xx + 663, yy + 177 + y2, string_hash_to_newline(L("Info:")));
            draw_set_font(fnt_40k_14);
            y2 += 20;
            draw_text_ext(xx + 663, yy + 177 + y2, string_hash_to_newline(string(p1)), -1, 469);
        }
        if (related[1] != "") {
            ...
            p1 += ".";
            draw_set_font(fnt_40k_14b);
            draw_text(xx + 663, yy + 177 + y2, string_hash_to_newline(L("See Also:")));
            draw_set_font(fnt_40k_14);
            y2 += 20;
            draw_text_ext(xx + 663, yy + 177 + y2, string_hash_to_newline(string(p1)), -1, 469);
        }
