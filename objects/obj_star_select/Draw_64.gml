...
try {
    ...
    var system_string = LF("{0} System", [target.name]);
    ...
    if (loading != 0) {
        ...
        draw_text(184, 202, L("Select Destination"));
    }
    ...
    if (_planet != 0) {
        ...
        if (feature != "") {
            ...
            if (garrison.garrison_force) {
                ...
                garrison_data_slate.sub_title = LF("Garrison Leader {0}", [garrison.garrison_leader.name_role()]);
                garrison_data_slate.body_text = garrison.garrison_report();

                garrison_data_slate.inside_method = function() {
                    garrison_data_slate.title = L("Garrison Report");
                    ...
                    var defence_string = LF("Planetary Defence : {0}", [defence_data[0]]);
                    ...
                    if (_dispo_change > 55) {
                        draw_text(xx + 20, half_way + 30, L("Garrison Disposition Effect : Positive"));
                    } else if (_dispo_change > 44 || _dispo_change == 0) {
                        draw_text(xx + 20, half_way + 30, L("Garrison Disposition Effect : Neutral"));
                    } else {
                        draw_text(xx + 20, half_way + 30, L("Garrison Disposition Effect : Negative"));
                    }
                };
                garrison_data_slate.draw(340 + main_data_slate.width, 160, 0.6, 0.6);
            }
        } else if (population) {
            garrison_data_slate.title = L("Population Report");
            garrison_data_slate.inside_method = function() {
        ...
        if (!debug) {
            for (var i = 0; i < array_length(buttons); i++) {
                if (shutters[i].draw_shutter(shutter_x, shutter_y + (47 * i), L(buttons[i]), 0.5, true)) {
    ...
    if (target != 0) {
        if ((player_fleet > 0) && (imperial_fleet + mechanicus_fleet + inquisitor_fleet + eldar_fleet + ork_fleet + tau_fleet + heretic_fleet > 0)) {
            ...
            draw_set_color(0);
            draw_set_font(fnt_40k_14b);
            draw_text(37, 413, L("Select Fleet Combat"));

            draw_set_color(CM_GREEN_COLOR);
            draw_set_font(fnt_40k_14b);
            draw_text(37.5, 413.5, L("Select Fleet Combat"));
