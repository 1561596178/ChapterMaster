...
actual_hi = string_width(L(unit_type)) * text_xscale;
if (actual_hi > max_hi) {
    repeat (10) {
        actual_hi = string_width(L(unit_type)) * text_xscale;
        if (actual_hi > max_hi) {
...
draw_text_transformed_outline(_x1 + (width / 2), _y1 + (height / 2), string(L(unit_type)), text_xscale, text_xscale, 270, c_black, c_white);
