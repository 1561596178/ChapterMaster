...
try {
    ...
    if (slide >= 2) {
        ...
        if (scr_hit(436, 74, 436 + 128, 74 + 128) && (popup == "")) {
            ...
            tooltip = L("Chapter Icon");
            tooltip2 = L("Your Chapter's icon.  Click to edit.");
        }
        ...
        if (slide == eCREATION_SLIDES.CHAPTERTRAITS) {
            ...
            if (scr_hit(1164 - 128, 74, 1164, 74 + 128)) {
                tooltip = L("Founding Chapter");
                tooltip2 = L("The parent Chapter whose gene-seed your own originates from.");
            }
    ...
    if (slide == eCREATION_SLIDES.CHAPTERTRAITS) {
        ...
        draw_text_transformed(580, 118, string_hash_to_newline(L("Successor Chapters: ") + string(successors)), 0.6, 0.6, 0);
        ...
        draw_set_halign(fa_center);
        draw_set_font(fnt_40k_30b);
        draw_text_transformed(800, 206, string_hash_to_newline(L("Gene-Seed Mutations")), 0.55, 0.55, 0);
        if (purity == 10) {
            draw_text_transformed(800, 230, L("The gene-seed is perfectly pure"), 0.5, 0.5, 0);
        } else {
            if (mutations > mutations_selected) {
                draw_text_transformed(800, 230, LF("Select {0} more, according to your purity score", [mutations - mutations_selected]), 0.5, 0.5, 0);
            } else {
                draw_text_transformed(800, 230, L("The gene-seed is mutated enough"), 0.5, 0.5, 0);
                draw_set_alpha(0.5);
            }
        }
        draw_set_halign(fa_left);
        ...
        for (var i = 0; i < array_length(mutations_defects); i++) {
            ...
            draw_text_transformed(x1 + 30, y1 + 4, L(mutation_data.t_tip), 0.4, 0.4, 0);
            if (scr_hit(x1, y1, x1 + 250, y1 + 20)) {
                tooltip = L(mutation_data.t_tip);
                tooltip2 = L(mutation_data.t_tip2);
            }
            y1 += spac;
            if (i == 6) {
                x1 = 750;
        ...
        draw_set_font(fnt_40k_30b);
        draw_set_halign(fa_center);
        draw_text_transformed(800, 515, string_hash_to_newline(L("Starting Disposition")), 0.6, 0.6, 0);

        draw_set_font(fnt_40k_14b);
        draw_set_halign(fa_right);

        draw_text(650, 550, string_hash_to_newline(L("Imperium") + " (" + string(disposition[2]) + ")"));
        draw_text(650, 575, string_hash_to_newline(L("Adeptus Mechanicus") + " (" + string(disposition[3]) + ")"));
        draw_text(650, 600, string_hash_to_newline(L("Ecclesiarchy") + " (" + string(disposition[5]) + ")"));
        draw_text(650, 625, string_hash_to_newline(L("Inquisition") + " (" + string(disposition[4]) + ")"));
        if (founding != ePROGENITOR.NONE) {
            draw_text(650, 650, string_hash_to_newline(L("Progenitor") + " (" + string(disposition[1]) + ")"));
        }
        draw_text(650, 675, L("Adeptus Astartes") + " (" + string(disposition[6]) + ")");
    ...
    if (slide == eCREATION_SLIDES.CHAPTERMASTER) {
        ...
        draw_set_color(CM_GREEN_COLOR);
        draw_set_halign(fa_left);
        draw_text_transformed(580, 100, string_hash_to_newline(L("Chapter Master Name: ")), 0.9, 0.9, 0);
        draw_set_font(fnt_40k_14b);
        ...
        draw_set_font(fnt_40k_30b);
        draw_text_transformed(444, 215, string_hash_to_newline(L("Select Two Weapons")), 0.6, 0.6, 0);
        draw_text_transformed(444, 240, string_hash_to_newline(L("Melee")), 0.6, 0.6, 0);
        draw_text_transformed(800, 240, string_hash_to_newline(L("Ranged")), 0.6, 0.6, 0);
        ...
        repeat (8) {
            ...
            draw_text_transformed(x6 + 30, y6 + 4, string_hash_to_newline(L(melee_choice_weapon)), 0.4, 0.4, 0);
            y6 += spac;
        }
        ...
        repeat (7) {
            ...
            draw_text_transformed(x6 + 30, y6 + 4, L(ranged_options[ranged_choice_order]), 0.4, 0.4, 0);
            y6 += spac;
        }
        ...
        repeat (3) {
            ...
            draw_text_transformed(x6 + 81, y6 + 214, L(cur_leader_type[0]), 0.5, 0.5, 0);
        ...
        if (custom != eCHAPTER_TYPE.PREMADE && global.chapter_id != eCHAPTERS.UNKNOWN) {
            ...
            draw_text_transformed(_sc_box.x1 + 90, _sc_box.y1 + 5, string(L("Save Chapter")), 0.6, 0.6, 0);
            draw_set_font(fnt_40k_14b);
            if (scr_hit(_sc_box.x1, _sc_box.y1, _sc_box.x2, _sc_box.y2)) {
                tooltip = L("Do you want to save your Chapter?");
                tooltip2 = L("Click to save your Chapter.");
                if (mouse_button_clicked()) {
                    scr_save_chapter(global.chapter_id);

                    tooltip = L("Do you want to save your Chapter?");
                    tooltip2 = L("Chapter Saved!");
                }
            }
        }
    }
    ...
    if (tooltip != "" && tooltip2 != "" && change_slide <= 0) {
        ...
        draw_set_halign(fa_left);
        draw_set_font(fnt_40k_14b);
        var _tt1 = L(string(tooltip));
        var _tt2 = L(string(tooltip2));
        var _width1 = string_width_ext(string_hash_to_newline(_tt1), -1, 500);
        draw_set_font(fnt_40k_14);
        var _width2 = string_width_ext(string_hash_to_newline(_tt2), 18, 500);
        var _height = string_height_ext(string_hash_to_newline(_tt2), 18, 500);

        var _box_w = max(_width1, _width2) + 24;
        var _box_x = mouse_x + 18;
        var _box_y = mouse_y + 20;
        if (_box_x + _box_w > room_width - 60) {
            _box_x = mouse_x - _box_w - 18;
        }
        if (_box_y + 44 + _height > room_height - 30) {
            _box_y = mouse_y - 44 - _height - 20;
        }

        draw_rectangle(_box_x, _box_y, _box_x + _box_w, _box_y + 44 + _height, 0);
        draw_set_color(CM_GREEN_COLOR);
        draw_rectangle(_box_x, _box_y, _box_x + _box_w, _box_y + 44 + _height, 1);
        draw_set_font(fnt_40k_14b);
        draw_text(_box_x + 4, _box_y + 2, string_hash_to_newline(string(_tt1)));
        draw_set_font(fnt_40k_14);
        draw_text_ext(_box_x + 4, _box_y + 22, string_hash_to_newline(string(_tt2)), 18, 500);
    }
} catch (_exception) {
