...
for (var i = 0, l = array_length(_command_staff); i < l; i++) {
    ...
    if (_hq_index.has_role(_officer.role)) {
        array_push(_parts, $"{L(_officer.prefix)}{L(_officer.role)} {obj_ini.name[0][_officer.name_slot]}");
    }
}
temp[34] = _build_clause(L("Command staff made of"), _parts);
...
for (var i = 0, l = array_length(_specialist_display); i < l; i++) {
    ...
    if (_count > 0) {
        array_push(_parts, $"{_count}x {L(_role_name)}");
    }
}
temp[35] = _build_clause(L("Specialist branches staffed by"), _parts);
...
if (_honour_guard_count > 0) {
    temp[35] += LF("\n\nHonour Guard, having the {0} most veteran {1} of your chapter serving in it.", [_honour_guard_count, L(string_plural("marine", _honour_guard_count))]);
}
...
for (var _com = 1; _com <= 10; _com++) {
    ...
    for (var i = 0, l = array_length(_keys); i < l; i++) {
        ...
        if (_count > 0) {
            array_push(_parts, $"{_count}x {L(_keys[i])}");
        }
    }
    for (var i = 0, l = array_length(_vehicle_display); i < l; i++) {
        var _count = _veh_counts[$ _vehicle_display[i]] ?? 0;
        if (_count > 0) {
            array_push(_parts, $"{_count}x {L(_vehicle_display[i])}");
        }
    }
    temp[35 + _com] = _build_clause(LF("{0} company made of", [L(integer_to_words(_com, true, true))]), _parts);
}

LOGGER.info("create Ships");

temp[59] = LF("CLASSIFICATION: SECTOR LOGISTICAE#++++++++++DATE: {0}#++++++++AUTHOR: MASTER ADEPT {1}#++++++++++++RE: INTRODUCTORY MISSIVE#+++++RECIPIENT: CHAPTER MASTER {2}##++THOUGHT: {3}++##I see you have made it unscathed, your grace. Death comes with you as it should! The enemy is on the horizon. Thy chapter is mighty and only waits for your word to wreak havoc upon our enemies.##Your chapter contains-", [temp[30], temp[31], temp[32], temp[33]]);
