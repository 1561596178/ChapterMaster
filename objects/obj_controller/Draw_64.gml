...
if (!zoomed && !zui) {
    ...
    if (!instance_exists(obj_popup)) {
        menu_buttons.chapter_manage.draw(34, 838 + y_slide, L("Chapter Management"), 1, 1, 145);
        menu_buttons.chapter_settings.draw(179, 838 + y_slide, L("Chapter Settings"), 1, 1, 145);
        menu_buttons.apoth.draw(357, 838 + y_slide, L("Apothecarium"));
        menu_buttons.reclu.draw(473, 838 + y_slide, L("Reclusium"));
        menu_buttons.lib.draw(590, 838 + y_slide, L("Librarium"));
        menu_buttons.arm.draw(706, 838 + y_slide, L("Armamentarium"));
        menu_buttons.recruit.draw(822, 838 + y_slide, L("Recruitment"));
        menu_buttons.fleet.draw(938, 838 + y_slide, L("Fleet"));
        menu_buttons.diplo.draw(1130, 838 + y_slide, L("Diplomacy"), 1, 1, 145);
        menu_buttons.event.draw(1275, 838 + y_slide, L("Event Log"), 1, 1, 145);
        menu_buttons.end_turn.draw(1420, 838 + y_slide, L("End Turn"), 1, 1, 145);
        menu_buttons.help.draw(1374, 8 + y_slide, L("Help"));
        menu_buttons.menu.draw(1484, 8 + y_slide, L("Menu"));
    }
    ...
    var _sector_string = LF("Sector {0}", [obj_ini.sector_name ?? "Terra Nova"]);
    ...
    if (faction_status[eFACTION.IMPERIUM] != "War") {
        if (penitent_max == 0) {
            draw_text(998, 17, L("Loyal"));
            draw_text(998, 17.5, L("Loyal"));
        }
        if (penitent_max > 0) {
            var endb2 = "";
            ...
            draw_text(998, 17, string(min(100, floor((penitent_current / penitent_max) * 100))) + "% " + L("Penitent"));
            draw_text(998, 17.5, string(min(100, floor((penitent_current / penitent_max) * 100))) + "% " + L("Penitent"));
            draw_set_color(CM_GREEN_COLOR);
    ...
    if (faction_status[eFACTION.IMPERIUM] == "War") {
        draw_set_color(255);
        draw_text(998, 17, L("Renegade"));
        draw_text(998, 17.5, L("Renegade"));
        draw_set_color(CM_GREEN_COLOR);
    }
    if (menu == eMENU.DEFAULT || menu == eMENU.TURN_END) {
        ...
        draw_text(850, 60, LF("Sector Fleet Strength {0}/{1}", [imp_ships, max_fleet_strength]));
        if (scr_hit([700, 60, 1000, 80])) {
            tooltip_draw(L("The relative strength of the imperial navy and defence fleet forces and their max supported strength. Increase The number of imperial aligned planets and active forge worlds to increase the limit"));
        }
    ...
    draw_text(36, 9, string(floor(requisition)) + string(inc));
    draw_text(36.5, 9.5, string(floor(requisition)) + string(inc));
    // Draws forge points
    draw_sprite_ext(spr_forge_points_icon, 0, 160, 15, 0.3, 0.3, 0, c_white, 1);
    draw_set_color(COL_FORGE_POINTS);
    draw_text(180, 9, string(forge_points));
    draw_text(180.5, 9.5, string(forge_points));
    ...
    draw_text(290, 9, string(loyalty));
    draw_text(290.5, 9.5, string(loyalty));
    // Draws the current gene seed
    draw_sprite(spr_new_resource, 2, 355, 17);
    draw_set_color(c_red);
    draw_text(370, 9, string(gene_seed));
    draw_text(370.5, 9.5, string(gene_seed));
    // Draws the current marines in your command
    draw_sprite(spr_new_resource, 3, 465, 17);
    draw_set_color(COL_REQUISITION);
    draw_text(485, 9, string(marines) + "/" + string(command));
    draw_text(485.5, 9.5, string(marines) + "/" + string(command));
    pop_draw_return_values();
}
