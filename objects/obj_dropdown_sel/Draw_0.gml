...
if ((scr_hit(x, y, x + width, y + height) == true) && (obj_controller.dropdown_open == 0)) {
    ...
    if (option_selected > 0) {
        if (option[option_selected] != "") {
            ...
            if ((target == "event_display") && (option[option_selected] == "None")) {
                tooltip = L("Display");
                tooltip2 = L("There is no Artifact set to be displayed at the event.");
            }
        }
    }
}
...
if (tooltip == "Great Feast") {
    tooltip2 = L("Holds a massive feast and celebration for your astartes.");
}
if ((tooltip != "Great Feast") && (target == "event_type")) {
    tooltip2 = L("(NOT COMPLETED YET)");
}
...
if ((tooltip != "") && (tooltip2 != "")) {
    ...
    draw_text(_box_x + 4, _box_y + 2, string_hash_to_newline(L(string(tooltip))));
    draw_set_font(fnt_40k_14);
    draw_text_ext(_box_x + 4, _box_y + 22, string_hash_to_newline(L(string(tooltip2))), 18, 500);
}
