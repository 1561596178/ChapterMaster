...
if (start == 0) {
    ...
    var _begin_y = 56;
    var _begin_text = L("[ Enter ] to Begin");
...
if (start < 7) {
    ...
    if (capital) {
        _stat_headers += L("Battleships:");
        _stat_numbers += string(capital);
    }
    if (frigate) {
        _newline = capital ? "\n" : "";
        _stat_headers += $"{_newline}{L("Cruisers:")}";
        _stat_numbers += $"{_newline}{string(frigate)}";
    }
    if (escort) {
        _newline = capital || frigate ? "\n" : "";
        _stat_headers += $"{_newline}{L("Escorts:")}";
...
if (start == 7) {
    ...
    if (_margin_percent >= 0.95) {
        _result_box_header = L("Major Victory");
    } else if (_margin_percent >= 0.75) {
        _result_box_header = L("Victory");
    } else if (_margin_percent > 0.01) {
        _result_box_header = L("Minor Victory");
    } else if (_margin_percent <= 0.01) {
        _result_box_header = L("Defeat");
    }
    var _result_box_footer = L("[ Enter ] to Continue");
    ...
    if (capital_max) {
        _result_box_text += $"\n{L("Battleships Lost:")} {string(capital_lost)} ({string((capital_lost / max(0, capital_max)) * 100)}%)";
    }
    if (frigate_max) {
        _result_box_text += $"\n{L("Cruisers Lost:")} {string(frigate_lost)} ({string((frigate_lost / max(0, frigate_max)) * 100)}%)";
    }
    if (escort_max) {
        _result_box_text += $"\n{L("Escorts Lost:")} {string(escort_lost)} ({string((escort_lost / max(0, escort_max)) * 100)}%)";
    }
    if (ships_max && (_all_lost > 0) && !_wiped_out) {
        _result_box_text += $"\n{L("Ships Damaged:")} {string(ships_damaged)} ({string((ships_damaged / (ships_max - _all_lost)) * 100)}%)";
    } else if (!_wiped_out) {
        _result_box_text += $"\n{L("Ships Damaged:")} 0 (0%)";
    }
    _result_box_text += $"\n{L("Marines Lost:")} {string(fallen)}";
